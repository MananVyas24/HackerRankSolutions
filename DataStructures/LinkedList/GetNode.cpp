...
Input Format 
You have to complete the int GetNode(Node* head, int positionFromTail) method which takes two arguments - the head of the linked list and the position of the node from the tail. 
...
int GetNode(Node *head,int positionFromTail)
{
    ...
    // Check positionFromTail lies inside the length of the LinkedList
    if (length < positionFromTail)
        return 0;
    
    temp = head;
    // * get positionFromTail-Length+1 from beginnning
    for (i=0 ; i < length-positionFromTail+1; i++)
        temp = temp->next;
    return  (temp->data);
}
*/
